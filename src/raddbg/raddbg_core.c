...
rd_init(CmdLine *cmdln)
{
  ...
  {
    Temp scratch = scratch_begin(0, 0);
    String8 user_program_log_path = os_get_process_info()->user_program_log_path;

    String8 user_log_folder = push_str8f(scratch.arena, "%S/raddbg", user_program_log_path);
    os_make_directory(user_log_folder);

    user_log_folder = push_str8f(scratch.arena, "%S/raddbg/logs", user_program_log_path);
